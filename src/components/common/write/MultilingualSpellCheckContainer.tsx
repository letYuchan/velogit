...
const MultilingualSpellCheckModal = ({
    ...
    const { data: languageList, refetch } = useGetAllAvailableLanguages();
    ...
    const getHighlightedText = (text: string, matches: Match[], focusIndex: number) => {
        ...
        sorted.forEach((m, i) => {
            const { offset, length, message } = m;
            if (offset > last) frags.push(text.slice(last, offset));

            const err = text.slice(offset, offset + length);
            ...
            <div className='w-full max-w-6xl rounded-2xl bg-background p-6 shadow-xl'>
                ...
                <div className='mb-6 grid grid-cols-1 gap-6 sm:grid-cols-2'>
                    <div className='max-h-[260px] overflow-y-auto whitespace-pre-wrap break-words rounded-md border border-border bg-backgroundDark p-4 text-foreground'>
                        {snapshotText
                            ? getHighlightedText(snapshotText, spellMatches, currentIndex)
